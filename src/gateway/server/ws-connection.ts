  ...
  wss.on("connection", (socket, upgradeReq) => {
    ...
    const send = (obj: unknown) => {
      try {
        ...
        // Diagnostic: log response sizes for node clients to identify oversized frames
        const maybeRole = (client?.connect as { role?: string } | undefined)?.role;
        if (maybeRole === "node") {
          const o = obj as { type?: string; id?: string; event?: string };
          const label = o.event ?? o.id ?? o.type ?? "?";
          console.log(
            `[ws:node-frame] conn=${connId.slice(0, 8)} ${label} ${frame.length}B`,
          );
          if (frame.length > 15360) {
            console.warn(
              `[ws:node-frame] ⚠️ OVERSIZED ${label} ${frame.length}B > 15KB limit!`,
            );
          }
        }
        socket.send(frame);
    ...
    socket.once("close", (code, reason) => {
      ...
      if (client?.connect?.role === "node") {
        console.log(
          `[ws:node-close] conn=${connId} code=${code} reason=${reason?.toString() || "n/a"} ` +
            `durationMs=${durationMs} cause=${closeCause ?? "n/a"} lastFrame=${lastFrameType}/${lastFrameMethod}/${lastFrameId} ` +
