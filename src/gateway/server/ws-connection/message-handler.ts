  ...
  socket.on("message", async (data) => {
    ...
    try {
      ...
      if (!client) {
        ...
        const isChromeExtension = connectParams.client.id === GATEWAY_CLIENT_IDS.CHROME_EXTENSION;
        ...
        // Chrome extension is a trusted companion — allow it to skip device identity
        // when shared-secret auth succeeds, same as control-ui bypass.
        const allowControlUiBypass =
          allowInsecureControlUi || disableControlUiDeviceAuth || isChromeExtension;
        ...
        if (!device) {
          ...
          if ((isControlUi || isChromeExtension) && !allowControlUiBypass) {
        ...
        // Node clients (e.g. ESP32 devices) have limited WebSocket buffers
        // (typically 15-64KB). The full hello-ok can be 300KB+ with ~650
        // method names, events, health/presence data. Strip it down for nodes.
        const isNodeClient = role === "node";
        const helloOk = {
          type: "hello-ok",
          ...
          server: {
            ...
            host: os.hostname(),
            connId,
            agentMode: process.env.HOC_AGENT_MODE === "1",
          },
          features: isNodeClient
            ? { methodCount: gatewayMethods.length, eventCount: events.length }
            : { methods: gatewayMethods, events },
          snapshot: isNodeClient
            ? {
            ...
            : snapshot,
          canvasHostUrl: isNodeClient ? undefined : canvasHostUrl,
          auth: deviceToken
            ? {
        ...
        if (role === "node") {
          ...
          // Mark node client with a stabilization window (3s) so broadcast()
          // skips presence/health/heartbeat events during the ESP32's
          // post-connect request burst, preventing TCP buffer overflow.
          (nextClient as { _nodeStabilizeUntil?: number })._nodeStabilizeUntil = Date.now() + 3000;
          ...
          // Delay voicewake push by 3s so the ESP32 can finish its
          // post-connect request burst (sessions, health, overview)
          // before receiving unsolicited events.
          setTimeout(() => {
            if (!context.nodeRegistry.get(nodeSession.nodeId)) {
