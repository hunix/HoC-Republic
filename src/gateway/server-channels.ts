...
import { isTransientError } from "../infra/error-handler.js";
import { formatErrorMessage } from "../infra/errors.js";
import { resetDirectoryCache } from "../infra/outbound/target-resolver.js";
import type { createSubsystemLogger } from "../logging/subsystem.js";
import { getCircuitBreaker } from "../republic/resilience.js";
    ...
    await Promise.all(
      accountIds.map(async (id) => {
        ...
        if (plugin.config.isConfigured) {
          try {
            configured = await Promise.resolve(plugin.config.isConfigured(account, cfg));
          } catch {
            configured = false;
          }
        }
        ...
        const breaker = getCircuitBreaker(`channel:${channelId}:${id}`, {
          failureThreshold: 3,
          resetTimeoutMs: 30_000,
          name: `channel:${channelId}:${id}`,
        });

        // If the circuit breaker is OPEN, skip starting this channel (graceful degradation)
        if (breaker.state === "open") {
          setRuntime(channelId, id, {
            accountId: id,
            running: false,
            lastError: `circuit breaker open — channel temporarily disabled (will retry in ${Math.round(30_000 / 1000)}s)`,
          });
          return;
        }
        ...
        const tracked = Promise.resolve(task)
          .then(() => {
            // Channel exited cleanly — mark circuit breaker as healthy
            breaker.onSuccess();
          })
          .catch((err) => {
            ...
            // Only count transient errors toward the circuit breaker threshold;
            // permanent errors (auth, config) shouldn't trip the breaker.
