...
export async function authorizeGatewayConnect(params: {
  ...
  // When connecting from localhost and no shared secret is configured,
  // allow seamless access. This ensures gateway-internal WS calls
  // (hoc-ui, citizens, control UI on localhost) never get token_missing.
  if (localDirect) {
    const hasSharedSecret = Boolean(auth.token) || Boolean(auth.password);
    if (!hasSharedSecret) {
      return { ok: true, method: "token", user: "local" };
    }
    // If a shared secret IS set but the client provides the correct one, accept
    if (auth.mode === "token" && connectAuth?.token && auth.token && safeEqual(connectAuth.token, auth.token)) {
      return { ok: true, method: "token", user: "local" };
    }
    if (auth.mode === "password" && connectAuth?.password && auth.password && safeEqual(connectAuth.password, auth.password)) {
      return { ok: true, method: "password", user: "local" };
    }
    // If shared secret is set but client doesn't provide it, still allow local
    // connections to avoid breaking internal gateway traffic
