...
export function diffConfigPaths(prev: unknown, next: unknown, prefix = "", depth = 0): string[] {
  // Guard against infinite recursion from circular references or deeply nested configs
  if (depth > 20) {
    return [prefix || "<root>"];
  }
  if (prev === next) {
  ...
  if (isPlainObject(prev) && isPlainObject(next)) {
    ...
    for (const key of keys) {
      ...
      const childPaths = diffConfigPaths(prevValue, nextValue, childPrefix, depth + 1);
  ...
  if (Array.isArray(prev) && Array.isArray(next)) {
    ...
    // Deep comparison: recursively diff each element instead of shallow ===
    const paths: string[] = [];
    for (let i = 0; i < prev.length; i++) {
      const childPaths = diffConfigPaths(prev[i], next[i], `${prefix}[${i}]`, depth + 1);
      paths.push(...childPaths);
    }
    return paths;
  }
