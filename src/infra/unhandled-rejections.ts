...
export function installUnhandledRejectionHandler(): void {
  process.on("unhandledRejection", (reason, _promise) => {
    ...
    if (isFatalError(reason)) {
      const msg = `[CRASH:FATAL] Unhandled rejection: ${formatUncaughtError(reason)}\n`;
      writeSync(1, msg);
    ...
    if (isConfigError(reason)) {
      const msg = `[CRASH:CONFIG] Unhandled rejection — configuration error: ${formatUncaughtError(reason)}\n`;
      writeSync(1, msg);
      console.error(msg.trim());
      process.exit(1);
      return;
    ...
    const msg = `[CRASH:UNHANDLED] Unhandled promise rejection (recovered): ${formatUncaughtError(reason)}\n`;
    writeSync(1, msg);
    console.error(msg.trim());
