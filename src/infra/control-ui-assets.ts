...
export function resolveControlUiRepoRoot(
  ...
  if (srcIndex !== -1) {
    ...
    if (fs.existsSync(path.join(root, "hoc-ui", "vite.config.ts"))) {
      return root;
    }
  }
  ...
  for (let i = 0; i < 8; i++) {
    if (
      ...
      fs.existsSync(path.join(dir, "hoc-ui", "vite.config.ts"))
    ) {
      return dir;
    }
...
const UI_SRC_EXTENSIONS = new Set([".ts", ".tsx", ".css", ".html", ".json", ".svg"]);

function collectSourceFiles(dir: string, exts: Set<string>): string[] {
...
/** Compute SHA-256 hash of all hoc-ui/src/** source files. */
export function computeUiSourceHash(repoRoot: string): string {
  const srcDir = path.join(repoRoot, "hoc-ui", "src");
  const files = collectSourceFiles(srcDir, UI_SRC_EXTENSIONS).toSorted();
  const hasher = crypto.createHash("sha256");
  for (const f of files) {
    hasher.update(path.relative(srcDir, f));
    hasher.update(fs.readFileSync(f));
  }
  return hasher.digest("hex").slice(0, 16);
}

export type BuildHashInfo = { hash: string; builtAt: string; fileCount: number };

/** Read the .build-hash file written by the UI build. */
export function readBuildHash(repoRoot: string): BuildHashInfo | null {
  const hashFile = path.join(repoRoot, "dist", "control-ui", ".build-hash");
  try {
    return JSON.parse(fs.readFileSync(hashFile, "utf8")) as BuildHashInfo;
  } catch {
    return null;
  }
}

/** Returns true if the UI source has changed since the last build. */
export function isControlUiBuildStale(repoRoot: string): boolean {
  const saved = readBuildHash(repoRoot);
  if (!saved) { return true; }
  const current = computeUiSourceHash(repoRoot);
  return current !== saved.hash;
}
...
export async function ensureControlUiAssetsBuilt(
  ...
  if (health.exists && repoRoot) {
    if (!isControlUiBuildStale(repoRoot)) {
      return { ok: true, built: false };
    }
    runtime.log("Control UI build is stale (source changed since last build); rebuilding…");
  } else if (health.exists) {
    // No repo root to check staleness — assume OK
    return { ok: true, built: false };
  }
