...
describe("installUnhandledRejectionHandler - fatal detection", () => {
  ...
  describe("fatal errors", () => {
    it("exits on ERR_OUT_OF_MEMORY", () => {
      ...
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringMatching(/\[CRASH:FATAL\].*Out of memory/),
      );
    });
  ...
  describe("configuration errors", () => {
    it("exits on INVALID_CONFIG", () => {
      ...
      expect(exitCalls).toEqual([1]);
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringMatching(/\[CRASH:CONFIG\].*Invalid config/),
      );
    });
  ...
  describe("non-fatal errors", () => {
    ...
    it("does NOT exit on generic errors without code (immortal gateway)", () => {
      ...
      expect(exitCalls).toEqual([]);
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        expect.stringMatching(/\[CRASH:UNHANDLED\].*Something went wrong/),
      );
    });
