...
import {
    addAllowlistEntry,
    analyzeArgvCommand, ensureExecApprovals, evaluateExecAllowlist,
    evaluateShellAllowlist, normalizeExecApprovals, readExecApprovalsSnapshot, recordAllowlistUse, requiresExecApproval, resolveExecApprovals, resolveExecApprovalsSocketPath, resolveSafeBins, saveExecApprovals, type ExecAllowlistEntry, type ExecApprovalsFile, type ExecAsk, type ExecCommandSegment, type ExecSecurity
...
import {
    enhancedKeyboardType, enhancedMouseClick, enhancedMouseMove, enhancedUIClick, enhancedUIFind, enhancedUIRead, executeEnhancedCommand, querySystemInfo,
    type EnhancedRunResult
} from "./runner-enhanced.js";
...
async function handleInvoke(
  ...
  if (command === "input.mouse.move") {
    try {
      const params = decodeParams<{ x: number; y: number }>(frame.paramsJSON);
      await enhancedMouseMove(params.x, params.y);
  ...
  if (command === "input.mouse.click") {
    try {
      const params = decodeParams<{ button?: "left" | "right" | "middle" }>(frame.paramsJSON);
      await enhancedMouseClick(params.button);
      await sendInvokeResult(client, frame, { ok: true });
    } catch (err) {
      await sendInvokeResult(client, frame, {
        ok: false,
  ...
  if (command === "input.keyboard.type") {
    try {
      const params = decodeParams<{ text: string }>(frame.paramsJSON);
      await enhancedKeyboardType(params.text);
      await sendInvokeResult(client, frame, { ok: true });
    } catch (err) {
      await sendInvokeResult(client, frame, {
        ok: false,
  ...
  if (command === "ui.automation.find") {
    try {
      const params = decodeParams<{ selector: string }>(frame.paramsJSON);
      const result = await enhancedUIFind(params.selector);
      await sendInvokeResult(client, frame, {
        ok: true,
  ...
  if (command === "ui.automation.click") {
    try {
      const params = decodeParams<{ selector: string }>(frame.paramsJSON);
      await enhancedUIClick(params.selector);
      await sendInvokeResult(client, frame, { ok: true });
    } catch (err) {
      await sendInvokeResult(client, frame, {
        ok: false,
  ...
  if (command === "ui.automation.read") {
    try {
      const params = decodeParams<{ selector: string }>(frame.paramsJSON);
      const text = await enhancedUIRead(params.selector);
      await sendInvokeResult(client, frame, {
        ok: true,
  ...
  if (command === "system.wmi.query") {
    try {
      const params = decodeParams<{ query: string }>(frame.paramsJSON);
      const results = await querySystemInfo(params.query);
      await sendInvokeResult(client, frame, {
        ok: true,
  ...
  // Use enhanced runner which supports companion service on Windows
  const result: EnhancedRunResult = await executeEnhancedCommand({
    command: execArgv[0],
    args: execArgv.slice(1),
    cwd: params.cwd?.trim() || undefined,
    env,
    timeout: params.timeoutMs ?? undefined
  });

  if (result.stdout.length > OUTPUT_CAP) {
    const suffix = "... (truncated)";
    result.stdout = result.stdout.slice(0, OUTPUT_CAP) + suffix;
  }
  if (result.stderr.length > OUTPUT_CAP) {
    const suffix = "... (truncated)";
    result.stderr = result.stderr.slice(0, OUTPUT_CAP) + suffix;
  }
