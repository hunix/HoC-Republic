...
export function createOpenClawCodingTools(options?: {
  ...
  // Citizen agents (cit_ prefix) have their tool configs seeded from simulation
  // persisted state — their allowlist warnings are not user config mistakes.
  const isCitizenAgent = typeof agentId === "string" && agentId.startsWith("cit_");
  const resolvePolicy = (policy: typeof profilePolicy, label: string) => {
    ...
    if (resolved.unknownAllowlist.length > 0) {
      ...
      if (isCitizenAgent) {
        logDebug(`tools: ${label} allowlist contains unknown entries (${entries}). ${suffix}`);
      } else {
