...
export function getShellConfig(): { shell: string; args: string[] } {
  if (process.platform === "win32") {
    // Try PowerShell first, but fallback to cmd.exe if PowerShell is not available
    // (e.g., Windows Server Core installations)
    ...
    // This ensures compatibility with Windows Server Core and minimal installations
    return {
      shell: "cmd.exe",
      args: ["/c"],
    };
