...
// Known reasoning model patterns — these models require thinking support even
// when the dynamic model catalog is empty, stale, or the entry lacks `reasoning: true`.
const KNOWN_REASONING_MODEL_PATTERNS: RegExp[] = [
  /^gpt-5/i,            // gpt-5, gpt-5-pro, gpt-5.1, gpt-5.2, etc.
  /^o[134]-/i,          // o1-mini, o1-preview, o3-mini, o3-pro, o4-mini, etc.
  /^o[134]$/i,          // bare o1, o3, o4
  /^deepseek-r/i,       // deepseek-r1, deepseek-r1-lite, etc.
];

function isKnownReasoningModel(provider: string, model: string): boolean {
  const normalizedProvider = normalizeProviderId(provider);
  // Only apply for providers known to host reasoning models
  if (!["openai", "openai-codex", "deepseek"].includes(normalizedProvider)) {
    return false;
  }
  const modelLower = model.trim().toLowerCase();
  return KNOWN_REASONING_MODEL_PATTERNS.some((pattern) => pattern.test(modelLower));
}
...
export function resolveThinkingDefault(params: {
  ...
  // Hardcoded fallback for known reasoning models when catalog is incomplete
  if (isKnownReasoningModel(params.provider, params.model)) {
    return "low";
  }
  return "off";
}
