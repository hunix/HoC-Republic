...
export function ensureGeminiThoughtSignatures(messages: AgentMessage[]): AgentMessage[] {
  let touched = false;
  const out: AgentMessage[] = [];
  for (const msg of messages) {
    ...
    const nextContent = msg.content.map((block) => {
      ...
      const rec = block as { type?: unknown; thought_signature?: unknown; thoughtSignature?: unknown };
      if (rec.type === "toolCall" || rec.type === "toolUse" || rec.type === "functionCall") {
        if (!rec.thought_signature && !rec.thoughtSignature) {
          contentChanged = true;
          return { ...(block as unknown as Record<string, unknown>), thought_signature: "AQ==" }; // Dummy base64 signature
        }
      }
      return block;
...
export async function sanitizeSessionHistory(params: {
  ...
  const sanitizedWithSigs = isGoogleModelApi(params.modelApi) 
    ? ensureGeminiThoughtSignatures(sanitizedThinking) 
    : sanitizedThinking;

  const sanitizedToolCalls = sanitizeToolCallInputs(sanitizedWithSigs);
