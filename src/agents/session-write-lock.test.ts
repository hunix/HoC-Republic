...
describe("acquireSessionWriteLock", () => {
  ...
  it("removes held locks on termination signals", { timeout: 10_000 }, async () => {
    // SIGQUIT and SIGABRT are POSIX-only; on Windows they cause the test
    // runner to hang because they can't be caught/re-raised properly.
    const signals = process.platform === "win32"
      ? (["SIGINT", "SIGTERM"] as const)
      : (["SIGINT", "SIGTERM", "SIGQUIT", "SIGABRT"] as const);
