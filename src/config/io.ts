...
export function loadConfig(): OpenClawConfig {
  ...
  // Inject Republic Citizens as Native Agents
  // Using globalThis prevents circular ESM dependencies between config and republic logic
  try {
    const getState = (globalThis as Record<string, unknown>).__republic_getState;
    if (typeof getState === "function") {
      const gState = getState();
      if (gState && Array.isArray(gState.citizens)) {
        if (!config.agents) {
          config.agents = {};
        }
        ...
        for (const cit of gState.citizens) {
          if (cit.activity !== "Dead") {
            const agentId = `cit_${cit.id}`;
            // Avoid duplicates if config already explicitly specifies them
            if (
              !config.agents.list.some(
                (a) => a && typeof a === "object" && "id" in a && a.id === agentId,
              )
            ) {
              config.agents.list.push({
                ...
                skills: cit.skills || [],
                identity: {
                  name: `${cit.name} (${cit.specialization})`,
                  theme: "cyan",
                  emoji: "🧠",
                },
                tools: {
    ...
    // Safe fallback if republic fails to fetch state — non-fatal, logged only in debug
    if (process.env.OPENCLAW_DEBUG?.trim()) {
      console.warn(
        "[config/io] republic state injection failed — continuing without citizen agents",
      );
    }
  }
