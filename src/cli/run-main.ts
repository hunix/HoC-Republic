...
export async function runCli(argv: string[] = process.argv) {
  ...
  process.on("uncaughtException", (error) => {
    const msg = `[CRASH:EXCEPTION] Uncaught exception: ${formatUncaughtError(error)}\n`;
    fs.writeSync(1, msg);  // writeSync is a blocking syscall — guaranteed to flush before exit
