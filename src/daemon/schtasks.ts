...
export async function restartScheduledTask({
  ...
  try {
    ...
    if (res.code !== 0) {
      const errorDetail = res.stderr || res.stdout;
      handleError(new Error(`schtasks run failed: ${errorDetail}`.trim()), {
        category: ErrorCategory.WINDOWS,
        severity: ErrorSeverity.ERROR,
        component: "scheduled-task",
        operation: "restart",
        metadata: { taskName, errorDetail },
      });
      throw new Error(`schtasks run failed: ${errorDetail}`.trim());
    }

    // Wait a moment for the task to actually start
    await new Promise((resolve) => setTimeout(resolve, 2000));

    // Verify the task is running
    const statusRes = await execSchtasks(["/Query", "/TN", taskName, "/V", "/FO", "LIST"]);
    if (statusRes.code === 0) {
      const parsed = parseSchtasksQuery(statusRes.stdout || "");
      const statusRaw = parsed.status?.toLowerCase();

      if (statusRaw !== "running") {
        const warning = `Task started but status is: ${parsed.status || "unknown"}`;
        handleError(new Error(warning), {
          category: ErrorCategory.WINDOWS,
          severity: ErrorSeverity.WARNING,
          component: "scheduled-task",
          operation: "restart-verify",
          metadata: {
            taskName,
