...
function foldMarkerChar(char: string): string {
  ...
  const angleSub = ANGLE_BRACKET_MAP[code];
  if (angleSub !== undefined) {
    return angleSub;
  }
  return char;
}
...
function replaceMarkers(content: string): string {
  ...
  // Match with optional id= attribute (from upstream random-ID markers) too
  const patterns: Array<{ regex: RegExp; value: string }> = [
    {
      regex: /<<<EXTERNAL_UNTRUSTED_CONTENT(?:\s+id="[a-f0-9]+")?>>>/gi,
      value: "[[MARKER_SANITIZED]]",
    },
    {
      regex: /<<<END_EXTERNAL_UNTRUSTED_CONTENT(?:\s+id="[a-f0-9]+")?>>>/gi,
...
export function wrapExternalContent(content: string, options: WrapExternalContentOptions): string {
  ...
  // Use unique random IDs on each marker pair to prevent spoofing by injected content
  const markerId = createExternalContentMarkerId();
  const startMarker = createExternalContentStartMarker(markerId);
  const endMarker = createExternalContentEndMarker(markerId);

  return [warningBlock, startMarker, metadata, "---", sanitized, endMarker].join("\n");
}
