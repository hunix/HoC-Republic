...
describe("external-content security", () => {
  ...
  describe("wrapExternalContent", () => {
    ...
    it("sanitizes boundary markers inside content", () => {
      ...
      const startMarkers = result.match(/<<<EXTERNAL_UNTRUSTED_CONTENT(?:\s+id="[a-f0-9]+")?>>>/g) ?? [];
      const endMarkers = result.match(/<<<END_EXTERNAL_UNTRUSTED_CONTENT(?:\s+id="[a-f0-9]+")?>>>/g) ?? [];
  ...
  describe("prompt injection scenarios", () => {
    ...
    it("safely wraps role hijacking attempt", () => {
      ...
      expect(result).toMatch(/<<<EXTERNAL_UNTRUSTED_CONTENT/);
      const markerIdx = result.search(/<<<EXTERNAL_UNTRUSTED_CONTENT/);
      expect(markerIdx).toBeLessThan(result.indexOf("</user>"));
    });
  });
});
