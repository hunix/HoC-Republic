...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  // Add a permanent error listener AFTER startup so post-listen server errors
  // (e.g. ECONNRESET, EPIPE, or a port collision on a subsequent bind attempt)
  // don't become uncaught exceptions that silently terminate the gateway process.
  server.on("error", (err) => {
    // Log the error but do NOT rethrow — the relay server is non-critical.
    const code = (err as NodeJS.ErrnoException).code ?? "UNKNOWN";
    console.error(`[extension-relay] server error (port=${info.port}, code=${code}): ${err.message}`);
  });
  wssExtension.on("error", (err) => {
    console.error(`[extension-relay] wssExtension error: ${(err as Error).message}`);
  });
  wssCdp.on("error", (err) => {
    console.error(`[extension-relay] wssCdp error: ${(err as Error).message}`);
  });
