...
export function scheduleConfigAutoSave(state: ConfigState) {
  if (autoSaveTimer) {clearTimeout(autoSaveTimer);}
  state.configAutoSaveStatus = "pending";
  autoSaveTimer = setTimeout(async () => {
    if (!state.configFormDirty) {return;}
    state.configAutoSaveStatus = "saving";
    await saveConfig(state);
    state.configAutoSaveStatus = state.lastError ? "error" : "saved";
    // Reset "saved" indicator after 2s
    setTimeout(() => {
      if (state.configAutoSaveStatus === "saved") {
        state.configAutoSaveStatus = null;
      }
    }, 2000);
  }, AUTO_SAVE_DELAY_MS);
}

export function cancelConfigAutoSave() {
  if (autoSaveTimer) {
