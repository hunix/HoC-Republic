...
export type DevicesState = {
  ...
  /** Last rotated token, for display in the UI instead of window.prompt() */
...
export async function rejectDevicePairing(
  ...
  // Callers should show their own inline confirmation UI and pass skipConfirm: true.
  // This guard is kept as a safety net for programmatic calls without UI confirmation.
  if (!opts.skipConfirm) {
    return;
  }
  try {
...
export async function rotateDeviceToken(
  ...
  try {
    ...
    if (res?.token) {
      ...
      // Store on state so the UI can display it inline instead of window.prompt()
      state.lastRotatedToken = res.token;
    }
    await loadDevices(state);
  } catch (err) {
    state.devicesError = String(err);
...
export async function revokeDeviceToken(
  ...
  // Inline confirmation must be shown in the view; pass skipConfirm: true after user confirms.
  if (!opts.skipConfirm) {
    return;
  }
  try {
