  ...
  return html`
    <section class="card">
      <div class="row" style="justify-content: space-between;">
        ...
        <button type="button" class="btn" ?disabled=${props.devicesLoading} @click=${props.onDevicesRefresh}>
  ...
  return html`
    <div class="republic-list__item">
      <div class="republic-list__left">
        <div class="republic-list__title">${name}</div>
        <div class="republic-list__meta">${req.deviceId}${ip}</div>
        <div class="republic-text--muted" style="margin-top: 6px;">
          ${role} · requested ${age}${repair}
        </div>
      </div>
      <div class="republic-list__right">
        <div class="row" style="justify-content: flex-end; gap: 8px; flex-wrap: wrap;">
          <button type="button" class="republic-btn republic-btn--sm republic-btn--primary" @click=${() => props.onDeviceApprove(req.requestId)}>
            Approve
          </button>
          <button type="button" class="republic-btn republic-btn--sm" @click=${() => props.onDeviceReject(req.requestId)}>
            Reject
          </button>
        </div>
      </div>
...
function renderPairedDevice(device: PairedDevice, props: NodesProps) {
  ...
  return html`
    <div class="republic-list__item">
      <div class="republic-list__left">
        <div class="republic-list__title">${name}</div>
        <div class="republic-list__meta">${device.deviceId}${ip}</div>
        <div class="republic-text--muted" style="margin-top: 6px;">${roles} · ${scopes}</div>
        ${
          tokens.length === 0
            ? html`
                <div class="republic-text--muted" style="margin-top: 6px">Tokens: none</div>
              `
            : html`
              <div class="republic-text--muted" style="margin-top: 10px;">Tokens</div>
      ...
      ${
        !state.ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button type="button" class="btn" ?disabled=${state.configLoading} @click=${state.onLoadConfig}>
      ...
      ${
        !ready
          ? html`<div class="row" style="margin-top: 12px; gap: 12px;">
            ...
            <button type="button" class="btn" ?disabled=${state.loading || !targetReady} @click=${state.onLoad}>
