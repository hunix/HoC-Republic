...
// ─── Collapsible panel persistence ──────────────────────────────
const PANEL_STORAGE_PREFIX = "hoc-cfg-panel-";

function isPanelOpen(key: string): boolean {
  try {
    const val = localStorage.getItem(PANEL_STORAGE_PREFIX + key);
    return val === null ? true : val === "1"; // open by default
  } catch { return true; }
}

function handlePanelToggle(key: string, e: Event) {
  const open = (e.target as HTMLDetailsElement).open;
  try { localStorage.setItem(PANEL_STORAGE_PREFIX + key, open ? "1" : "0"); } catch { /* noop */ }
}
        ...
        subsectionContext
          ? (() => {
              ...
              <details
                ...
                id=${id}
                ?open=${isPanelOpen(`${sectionKey}-${subsectionKey}`)}
                @toggle=${(e: Event) => handlePanelToggle(`${sectionKey}-${subsectionKey}`, e)}
              >
                <summary class="config-section-card__header">
                  ...
                  <svg class="config-section-card__chevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          : filteredEntries.map(([key, node]) => {
              ...
              <details
                ...
                ?open=${isPanelOpen(key)}
                @toggle=${(e: Event) => handlePanelToggle(key, e)}
              >
                <summary class="config-section-card__header">
