...
beforeEach(() => {
  ...
  window.__HOC_CONTROL_UI_BASE_PATH__ = undefined;
  localStorage.clear();
...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/hoc/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/hoc");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/hoc/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__HOC_CONTROL_UI_BASE_PATH__ = "/hoc";
    const app = mountApp("/hoc/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/hoc");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/hoc/sessions");
  });
