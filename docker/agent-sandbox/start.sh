#!/bin/bash
# ─── HoC Agent Sandbox Entrypoint ────────────────────────────────
# Starts: Xvfb (virtual display) → Fluxbox (WM) → x11vnc (VNC) →
#         noVNC (browser client) → Sandbox API → Preview server
set -e

echo "🚀 HoC Agent Sandbox starting..."
