"""
HoC Agent Sandbox — Internal API Server

Lightweight FastAPI service running inside the sandbox container.
...
app = FastAPI(title="HoC Agent Sandbox API", version="1.0.0")
...
async def take_screenshot():
    ...
    # Using scrot for extremely fast screenshots compared to xwd
    proc = await asyncio.create_subprocess_shell(
...
async def computer_action(req: ComputerActionRequest):
    ...
    if action == "mouse_move" and req.coordinate:
        ...
        # Escape quotes for bash execution safely
    ...
    # Introduce small delay for UI to settle before taking validation snapshot
    time.sleep(0.5)
    return await take_screenshot()
