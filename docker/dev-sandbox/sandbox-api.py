"""
HoC Dev Sandbox — Internal API Server

Lightweight FastAPI service running inside the dev sandbox container.
The gateway calls this to execute commands, run scripts, and control the browser.
Same interface as the standard agent-sandbox API.
"""
...
app = FastAPI(title="HoC Dev Sandbox API", version="1.0.0")
...
@app.get("/health")
async def health():
    """Health check — includes all tool versions."""
    import shutil
    disk = shutil.disk_usage("/workspace")

    # Collect tool versions (best-effort)
    def get_version(cmd: str) -> str:
        try:
            return subprocess.check_output(cmd, shell=True, timeout=3, stderr=subprocess.STDOUT).decode().strip().split("\n")[0]
        except Exception:
            return "unavailable"

    return {
        "ok": True,
        "service": "hoc-dev-sandbox",
        "workspace_disk_free_gb": round(disk.free / 1e9, 2),
        "python_version": sys.version.split()[0],
        "tools": {
            "node": get_version("node --version"),
            "pnpm": get_version("pnpm --version"),
            "go": get_version("go version 2>/dev/null | awk '{print $3}'"),
            "rust": get_version("rustc --version 2>/dev/null | awk '{print $2}'"),
            "claude": get_version("claude --version 2>/dev/null || echo 'installed'"),
            "gh": get_version("gh --version 2>/dev/null | head -1"),
            "docker": get_version("docker --version 2>/dev/null"),
            "supabase": get_version("supabase --version 2>/dev/null"),
            "deno": get_version("deno --version 2>/dev/null | head -1"),
            "code_server": get_version("code-server --version 2>/dev/null | head -1"),
        },
        "pid": os.getpid(),
    }
