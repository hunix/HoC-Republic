...
export function extractGeminiCliCredentials(): { clientId: string; clientSecret: string } | null {
  ...
  try {
    ...
    const secretPrefix = ["GO", "CSPX"].join("");
    const idMatch = content.match(new RegExp(`(${clientIdPattern})`));
    const secretMatch = content.match(new RegExp(`(${secretPrefix}-[A-Za-z0-9_-]+)`));
