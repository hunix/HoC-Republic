...
export const whatsappPlugin: ChannelPlugin<ResolvedWhatsAppAccount> = {
  ...
  config: {
    ...
    // isConfigured: check if Baileys auth directory exists on disk.
    // IMPORTANT: do NOT call getWhatsAppRuntime() here — the runtime is only
    // initialized AFTER the channel is started (startAccount). Calling it
    // here during gateway startup throws 'WhatsApp runtime not initialized',
    // which causes an unhandled rejection and process.exit(1).
    isConfigured: (account) => {
      const dir = account.authDir;
      if (!dir) {
        return false;
      }
      try {
