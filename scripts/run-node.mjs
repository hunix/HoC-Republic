...
const logRunner = (message) => {
  ...
  process.stderr.write(`[hoc] ${message}\n`);
};
...
const runNode = () => {
  const nodeProcess = spawn(
    process.execPath,
    [
      "--max-old-space-size=8192", // 8 GB heap limit — system has 68 GB RAM
      "--expose-gc",               // enable global.gc() for memory pressure manager hints
      "openclaw.mjs",
      ...args,
    ],
    {
  ...
  nodeProcess.on("exit", (exitCode, exitSignal) => {
    ...
    process.stderr.write(`\n[run-node ${ts}] child exited: code=${exitCode} signal=${exitSignal}\n`);
    if (exitSignal) {
      process.exit(1);
    }
